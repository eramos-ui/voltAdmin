...
import { GridRow } from "@/app/components/controls/grid/GridRow";
...
export type CustomGridProps<T> = {
  ...
  isEditable?: (column: ColumnConfig<T>, row: T) => boolean;
};
...
export const CustomGrid = <T,>({
  ...
  const getEditableState = (column: ColumnConfig<T>, row: T) => {
    if (isEditable) {

...
import { FormConfig, FormValues, FormField, GridColumn, GridRow  } from '../../../types/interfaces';
...
interface Props {
  field: FormField;  // Usamos FormField en lugar de definir cada propiedad por separado
}
interface Props {
  titleGrid: string;
  labelGridAdd:string;
  label: string;  name: string;  columns: GridColumn[];  rows: GridRow[];  actions: ('add' | 'edit' | 'delete')[];
  ...
  marginBottom?: string;  editFormConfig?: FormConfig;  theme?: 'light' | 'dark';  
...
export const MyGrid: React.FC<Props> = ({  label, titleGrid, labelGridAdd, objectGrid,
  ...
  const { setFieldValue }                       = useFormikContext<{ [key: string]: GridRow[] }>();
  const [ gridRows, setGridRows ]               = useState<GridRow[]>(rows || []);
  ...
  const isUniqueValue = (newRow: GridRow, key: string): boolean => {//gridRows son las filas que hay en la grilla y newRow es la que se agrega
  ...
  const handleFormSubmit = (values: FormValues, { setSubmitting }: FormikHelpers<FormValues>) => {//values son los de la fila que se edita o agrega
    const newRow: GridRow = {}; //para mapear los valores del formulario a una fila
    ...
    if (isAdding) {   
      ...
      const newRows = [...gridRows, values as GridRow];
      setGridRows(newRows);
      setFieldValue(name, newRows);
    } else if (editingRowIndex !== null) {
      const newRows = [...gridRows];
      newRows[editingRowIndex] = values as GridRow;
      setGridRows(newRows);
      setFieldValue(name, newRows);
      setEditingRowIndex(null);
    }
  ...
  return (
    <>
    {/* {console.log('en render rows',gridRows,columns)} */}
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          {actions?.includes('add') && (
            <button type="button"  onClick={handleAdd}
              className="flex items-center px-2 py-1 text-sm font-medium text-white bg-blue-500 rounded hover:bg-blue-600"
            >
        ...
        <EditForm isOpen={isModalOpen} onClose={() => reLoad()  } theme={props.theme}
          initialValues={editingRowIndex !== null ? gridRows[editingRowIndex] : {}} onSubmit={handleFormSubmit} 
